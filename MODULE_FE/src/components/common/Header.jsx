...
export const Header = () => {
  ...
  // Lấy user thật từ Redux thay vì dùng biến ảo
  const { user } = useSelector((state) => state.auth);

  // Xử lý lấy chữ cái đầu của tên để làm Avatar (ví dụ: Admin Manager -> AM)
      ...
      <div className="mx-auto h-full px-4 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden lg:flex items-center gap-1 border-l border-slate-200 pl-6">
              {navItems.map((item) => (
                <Link 
                  key={item.path}
                  to={item.path} 
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="relative">
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 px-2 py-1 rounded-full hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-teal-600 to-teal-500 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                  {getInitials(user?.fullname)}
                </div>
                <span className="hidden sm:inline text-sm font-medium text-slate-900">{user?.fullname || 'Cửa hàng'}</span>
