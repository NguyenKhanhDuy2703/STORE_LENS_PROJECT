...
const ManagermentCameraPage = () => {
  ...
  useEffect(() => {
    if (locationId && locationId !== 'all') {
      dispatch(fetchCameraDashboardThunk(locationId));
    }
  }, [dispatch, locationId]);
  ...
  const openCreateModal = () => {
    if (!locationId || locationId === 'all') {
      showWarningAlert({
        title: 'Thiếu vị trí',
        text: 'Vui lòng chọn cửa hàng trước khi thêm camera.',
      });
      return;
    }
  ...
  const openEditModal = (camera) => {
    if (!locationId || locationId === 'all') {
      showWarningAlert({
        title: 'Thiếu vị trí',
        text: 'Vui lòng chọn cửa hàng trước khi cập nhật camera.',
      });
      return;
    }
  ...
  const handleUpsertCamera = async ({ cameraCode, cameraData }) => {
    ...
    try {
      ...
      showCompactSuccessAlert({
        ...
        text: isEditMode
          ? 'Thông tin camera đã được cập nhật thành công.'
          : 'Camera mới đã được lưu thành công.',
      });

      await dispatch(fetchCameraDashboardThunk(locationId)).unwrap();
    } catch (upsertError) {
      showCompactErrorAlert({
  ...
  const handleDeleteCamera = async (cameraCode) => {
    ...
    try {
      ...
      showCompactSuccessAlert({
        title: 'Đã xóa',
        text: 'Camera đã được xóa thành công.',
      });
      ...
      showCompactErrorAlert({
        title: 'Xóa thất bại',
        text: deleteError || 'Không thể xóa camera.',
      });
    }
  };
  ...
  const handleToggleCameraPower = async (camera) => {
    ...
    if (!cameraCode || !urlRtsp || !locationId || locationId === 'all') {
      showCompactErrorAlert({
        title: 'Thiếu dữ liệu',
        text: 'Không đủ thông tin camera để thực hiện bật/tắt.',
      });
      return;
    }
    ...
    try {
      if (isCameraRunning) {
        ...
        await dispatch(turnOnCameraThunk({ cameraCode, urlRtsp, locationId })).unwrap();
      }

      showCompactSuccessAlert({
        title: isCameraRunning ? 'Đã tắt camera' : 'Đã bật camera',
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6">
        <div className="text-center">
          <div className="mb-3 inline-block h-10 w-10 animate-spin rounded-full border-4 border-indigo-200 border-t-indigo-600"></div>
